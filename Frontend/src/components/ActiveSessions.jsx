...
// Updated: Added isSessionFull prop
        ...
        <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
          {isLoading ? (
            ...
            sessions.map((session) => {
              // Determine if the current user can join or if the room is locked
              ...
              return (
                <div
                  ...
                  className={`card bg-base-200 border-2 transition-all ${
                    cannotJoin 
                      ? "opacity-75 border-transparent grayscale-[0.5]" 
                ...
                >
                  <div className="flex items-center justify-between gap-4 p-5">
                    ...
                    {/* ACTION BUTTON */}
                    {cannotJoin ? (
                      <button className="btn btn-disabled btn-sm px-6">
                        Full
                      </button>
                    ) : (
                      <Link to={`/session/${session._id}`} className="btn btn-primary btn-sm gap-2 px-6">
            ...
            <div className="text-center py-16">
              <div className="w-20 h-20 mx-auto mb-4 bg-gradient-to-br from-primary/20 to-secondary/20 rounded-3xl flex items-center justify-center">
                <SquareTerminal className="w-10 h-10 text-primary/50" />
              </div>
